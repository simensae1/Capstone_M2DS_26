...
def route_individual_pair(pair, graph_gdf_segments, angle_map, edges_dict, params, initial_pheromones):
    ...
    # NEW: Keep track of the best path known at each iteration
    path_history = []
    ...
    for _ in range(n_iterations):
        ...
        # NEW: Append a copy of the best path found so far
        path_history.append(best_path.copy() if best_path else None)

    # NEW: Return the path_history as well
    return pair, best_path, best_dist, path_history

# ==========================================
# 2. HELPER FUNCTIONS & REPAIR LOGIC
# ==========================================
def plot_convergence(trench_history, dijkstra_trench, title):
    """
    Plots the global trench length over ACO iterations compared to the Dijkstra baseline.
    """
    plt.figure(figsize=(10, 6))
    plt.plot(range(len(trench_history)), trench_history, label='ACO Trench Length', color='red', linewidth=2)
    plt.axhline(y=dijkstra_trench, color='blue', linestyle='--', linewidth=2, 
                label=f'Dijkstra Baseline ({dijkstra_trench:.2f}m)')
    
    plt.xlabel('Iteration Number', fontsize=12)
    plt.ylabel('Global Trench Length (m)', fontsize=12)
    ...
    safe_title = title.replace('=', '').replace(',', '').replace(' ', '_').replace('(', '').replace(')', '')
...
# --- ACO EXECUTION (SINGLE RUN) ---
    print("\nStarting Ant Colony Optimization (ACO)...")

    # 1. Define your single set of parameters
    a = 0.5  # Alpha: Pheromone importance
    b = 1.0  # Beta: Distance importance
    g = 0.5  # Gamma: Angle (straightness) importance
    n_ants = 70
    n_iterations = 500
    ...
    # Extract successful routes and their iteration histories
    all_routes = {p: path for p, path, d, hist in results if path}
    all_histories = {p: hist for p, path, d, hist in results}
    ...
    # ---------------------------------------------------------
    # CALCULATE AND PLOT TRENCH LENGTH PER ITERATION
    # ---------------------------------------------------------
    trench_lengths_per_it = []
    for i in range(n_iterations):
        unique_edges_it = set()
        for pair, hist in all_histories.items():
            # Get the path this pair was using at iteration i
            if i < len(hist) and hist[i] is not None:
                path = hist[i]
                for j in range(len(path)-1):
                    unique_edges_it.add(tuple(sorted((path[j], path[j+1]))))
        
        # Sum the physical lengths of all unique edges used globally at iteration i
        trench_m = sum(edges_dict[e] for e in unique_edges_it)
        trench_lengths_per_it.append(trench_m)
        
    # Generate the convergence plot
    convergence_title = f"Convergence Alpha={a} Beta={b} Gamma={g}"
    plot_convergence(trench_lengths_per_it, dijkstra_metrics["Trench_m"], title=convergence_title)
    # ---------------------------------------------------------

    # Dynamic Plot Title for spatial layout
    ...
    csv_filename = "aco_single_run_results.csv"
    df_results.to_csv(csv_filename, index=False)

    print(f"\nExecution complete! Results saved to '{csv_filename}'.")
        ...
        for _ in range(n_ants):
            ...
            for _ in range(len(edges_dict)): 
                ...
                if next_node in path:
                    idx = path.index(next_node); path = path[:idx + 1]
                    current = next_node; prev = path[-2] if len(path) > 1 else None
                else:
...
def plot_final_network(graph_data, all_routes):
    edge_counts = Counter()
    for path in all_routes.values():
        for i in range(len(path)-1): edge_counts[tuple(sorted((path[i], path[i+1])))] += 1
